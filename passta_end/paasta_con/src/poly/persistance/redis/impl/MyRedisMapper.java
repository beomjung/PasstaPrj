...
public class MyRedisMapper implements IMyRedisMapper{
	...
	public void doSaveDataforListJSON() throws Exception {
		...
		String redisKey = "Test02-List-JOSON";
		
		/*
		 * redis 저장 및 읽기에 대한 데이터 타입지정(String 타입으로 지정
		 */
		redisDB.setKeySerializer(new StringRedisSerializer()); // String 타입
		...
		if(redisDB.hasKey(redisKey)) {
			...
			// Redis에 저장된 데이터 전체 가져오기
			// 데이터 인덱스는 0부터 시작하며, 세번째 인자값은 01로 설정하면 모두 가져옴
			...
			while(it.hasNext()) {
				...
				log.info("name : " + CmmUtil.nvl(rDTO.getEmail()));
				log.info("name : " + CmmUtil.nvl(rDTO.getAddr()));
			}
		} else {
	...
	public void doSavaDataforSet() throws Exception {
		...
		/*
		 * redis 저장 및 읽기에대한 데이터 타입 지정(String 타입으로 지정함)
		 */
		redisDB.setKeySerializer(new StringRedisSerializer());
		redisDB.setValueSerializer(new StringRedisSerializer());
	...
	public void doSavaDataforZSet() throws Exception {
		...
		log.info(this.getClass().getName() + ".doSavaDataforZSet start!!");
		...
		log.info(this.getClass().getName() + ".doSavaDataforZSet end!!");
