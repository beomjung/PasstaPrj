...
public class MelonService implements IMelonService{
	...
	public int collectMelonSong() throws Exception {
		...
		log.info("collectMelonSong Start");
		...
		// 멜론 Top100중 50위 까지 정보 가져오는 페이지
		String url = "https://www.melon.com/chart/day/index.htm";
		
		// JSOUP 라이브러리를 통해 사이트 접속되면, 그사이트의 전체 HTML소를 저장할 변수
		Document doc = null;
		
		doc = Jsoup.connect(url).get();
		
		// <div class="service_list_song"> 태그 내에서 있는 HTML 소스만 element에 저장됨
		Elements element = doc.select("div.service_list_song");
		
		// Iterator를 사용하여 멜론차트 정보 가져오기
		Iterator<Element> songList = element.select("div.wrap_song_info").iterator(); // 멜론 50위까지 차트
	...
	public List<Map<String, Object>> getSingerSongCnt() throws Exception {
		...
		log.info("getSingerSongCnt Start");
		...
		// DTO를 전달하게 되면, Null 값이 변수들까지 출력되는 문제가 있어 Map을 활용함
